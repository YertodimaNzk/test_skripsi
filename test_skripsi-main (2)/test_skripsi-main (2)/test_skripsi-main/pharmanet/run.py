...
# Route untuk menyimpan data about us yang diperbarui ke database
    ...
    if request.method == 'POST':
        try:
            ...
            query = """
                ...
                WHERE nama = %s
            """
            cursor.execute(query, (deskripsi, nama, no_telephone, sosial_media, nama))
            db.commit()
            cursor.close()
            ...
            print("Error updating about us:", str(e)) 
            response = {'status': 'error', 'message': 'Gagal memperbarui data: ' + str(e)}
            return jsonify(response), 500

# Route untuk menghapus data about us dari database
...
def deleteAboutUs():
    if request.method == 'POST':
        ...
        cursor = db.cursor()
        query = "DELETE FROM about_us WHERE nama = %s"
